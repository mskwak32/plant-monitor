...
`DB_PATH`: `Path(__file__)`로 절대 경로 계산 → `plant_monitor_rpi/plant_monitor.db`. `get_connection()`: `sqlite3.connect()` + `row_factory = sqlite3.Row` 설정. `init_db()`: `executescript()`로 3개 테이블 생성. settings는 `CHECK (id = 1)` 싱글턴, `INSERT OR IGNORE`로 기본값 삽입.

**`db/repository.py`**

계층별 CRUD. `insert_sensor(SensorData)`, `get_latest_sensor()`, `get_sensor_history(limit=100)`, `insert_pump(PumpState)`, `get_settings()`, `update_soil_min(value)`. 모두 `with get_connection() as conn` 패턴 사용. 컬럼명 `soil_moisture_min`으로 통일 (`soil_humidity_min` 아님).

**`service/uart_listener.py`**

모듈 수준 함수 두 개로 구성. `_read_loop(port, queue, loop)`: daemon 스레드에서 실행, `readline()` 블로킹 루프, `parse_line()` 후 `insert_sensor()`/`insert_pump()` DB 저장 (try/except로 루프 보호), `loop.call_soon_threadsafe(queue.put_nowait, parsed)`로 이벤트 루프에 위임. `start(port, queue)`: `asyncio.get_event_loop()`로 루프 캡처 후 `daemon=True, name="uart-listener"` 스레드 시작.
